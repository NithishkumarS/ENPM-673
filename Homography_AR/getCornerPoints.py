...
def sortCorners(__corners__):
    ...
    s = __corners__.sum(axis=1)

    rect[0] = __corners__[np.argmin(s)]
    __corners__ = np.delete(__corners__, np.argmin(s), axis=0)
    s = __corners__.sum(axis=1)
    rect[2] = __corners__[np.argmax(s)]
    __corners__ = np.delete(__corners__, np.argmax(s), axis=0)

    diff = np.diff(__corners__, axis=1)
    rect[1] = __corners__[np.argmin(diff)]
    __corners__ = np.delete(__corners__, np.argmin(diff), axis=0)
    diff = np.diff(__corners__, axis=1)
    rect[3] = __corners__[np.argmax(diff)]
