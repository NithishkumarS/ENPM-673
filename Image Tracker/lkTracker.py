...
#     img = np.copy(frame)
#     ploti = cv2.rectangle(img,(cornerPoints[0][0],cornerPoints[0][1]),(cornerPoints[2][0],cornerPoints[2][1]),[0,0,255])
#     cv2.imshow('After Warp frame', ploti)
       
    cv2.imshow('After warp',frame)
    '''
    input_frame = cv2.polylines(frame,[cornerPoints],True,(0))
    template = cv2.polylines(tmpImg,[cornerPoints],True,(0))
    '''
...
#     cv2.imshow('steepest descent',hessian2)
